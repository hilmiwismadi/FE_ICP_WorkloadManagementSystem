...
export default function CreateTaskModal({
  ...
  const handleConfirmedSubmit = async () => {
    ...
    try {
      ...
      // Prepare initial email data before API calls to validate it
      const employeeEmails = selectedEmployees
        .flatMap((emp) => emp.users?.map((user) => user.email) || [])
        .filter((email): email is string => email !== undefined);

      if (employeeEmails.length === 0) {
        throw new Error(
          "No valid email addresses found for selected employees"
        );
      }
              ...
              <div className="relative">
                <div
                  className={cn(
                    ...
                    isDropdownOpen &&
                      "ring-[0.104vw] ring-blue-500 border-blue-500"
                  )}
