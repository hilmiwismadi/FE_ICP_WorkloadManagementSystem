  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const taskData = {
        ...
        workload: parseFloat(formData.workload.toString()),
        start_Date: new Date(formData.startDate).toISOString(),
        end_Date: new Date(formData.endDate).toISOString(),
        employee_Id: params.id
      };

      const response = await fetch(`https://be-icpworkloadmanagementsystem.up.railway.app/api/task/add/${token}`, {
        method: 'POST',
        headers: {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="p-[1vw] overflow-hidden rounded-[0.833vw]">
        <form onSubmit={handleSubmit} className="p-[1.25vw] space-y-[1.25vw]">
        <DialogTitle className='text-[1.33vw] font-bold pt-[1.25vw]'>Add new task</DialogTitle>
